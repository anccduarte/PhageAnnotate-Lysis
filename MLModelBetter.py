...
class MLModel:
    ...
    def __init__(self, model: str, dataset: str) -> None:
        ...
        # name of the model and data
        self.model = model
        self.dataset = pd.read_csv(dataset)
        self.ml_data = self.__scale_data()
        
        # hyperparameter optimization
        self.hyper_l = {"RF": {"max_features": ["sqrt", "log2"], "n_estimators": [50, 100, 200]},
                        "SVM": {"C": [0.1, 1], "kernel": ["linear", "poly"], "gamma": ["auto", "scale"]},
                        "ANN": {"hidden_layer_sizes": [(10,), (50,)], "alpha": [0.0001, 0.001, 0.01]}}
        self.hyper_r = {"RF": {"max_features": ["sqrt", "log2"], "n_estimators": [50, 100, 150, 200],
    ...
    def __optimize_hyperparameters(self, mode: str) -> dict:
        ...
        if mode == "light": param_grid = self.hyper_l[self.model]
        else: param_grid = self.hyper_r[self.model]
        # seleção do 'estimator'
        if self.model == "RF": estimator = RandomForestClassifier()
        elif self.model == "SVM": estimator = SVC()
        else: estimator = MLPClassifier()
        # determinação dos hiperparâmetros ótimos (5-fold cross validation (cv) -> default)
        grid = GridSearchCV(estimator=estimator, param_grid=param_grid)
        grid.fit(x_train, y_train)
        return grid.best_params_
    
    def __get_estimator(self, mode: str) -> Union[RandomForestClassifier, SVC, MLPClassifier]:
        """
    ...
    def __build_model(self) -> None:
        ...
        self.mask = self.__get_feature_mask(estimator1) # também é utilizado em self.predict_proteins()
